...
func (l *Listener) Close() error {
	...
	l.closeOnce.Do(func() {
		...
		if l.remoteSigner != nil {
			if err := l.remoteSigner.Close(); err != nil {
				log.Debug().Err(err).Str("lease_id", l.lease.ID).Msg("[SDK] Failed to close remote signer")
			}
		}
...
// SetTLSConfig activates keyless TLS mode on this listener.
func (l *Listener) SetTLSConfig(tlsConfig *tls.Config, remoteSigner io.Closer) {
	l.mu.Lock()
	defer l.mu.Unlock()
	l.tlsConfig = tlsConfig
	l.remoteSigner = remoteSigner
}
